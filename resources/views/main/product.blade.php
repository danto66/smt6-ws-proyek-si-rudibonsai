...
<div class="min-h-screen mt-6 px-2 sm:px-8 xl:px-4 max-w-7xl mx-auto">
    ...
    <div class="sm:w-2/3 sm:mx-auto mt-4 flex justify-center space-x-2">
        <form class="w-full" method="POST" action="{{ route('main.products.search') }}">
            @csrf
            <div x-data="{searchHover:false}" :class="{'bg-pink-700' : searchHover}"
                class="h-full flex flex-auto justify-center bg-pink-400 rounded-md ">
                <x-input :value="request()->routeIs('main.products.search') ? $keyword: ''"
                    placeholder="Cari hijab apa?" class="w-full" type="search" name="keyword_product"
                    id="keyword_product" />
        ...
        <div x-data="{open : false}" class="relative">
            <button @click="open = true" class="px-4 py-3 rounded-md bg-pink-400 text-white hover:bg-pink-700">
    ...
    @if ($products->count() < 1) <div class="mt-6 flex justify-center w-full bg-white p-8 rounded shadow">
