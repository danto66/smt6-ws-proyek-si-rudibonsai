...
@section('content')
    <div class="mt-5 px-2 sm:px-8 xl:px-4 max-w-2xl mx-auto min-h-screen">
        ...
        <div class="bg-white overflow-auto whitespace-nowrap mt-4 p-7 rounded-md shadow ">
            <form class="mt-8 space-y-6" action="{{ route('admin.admin-account.update') }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input required id="password" name="password" type="password" autocomplete="fullpassword" minlength="7"
                ...
                <div>
                    <button type="submit"
                        class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-blue-500 hover:border-blue-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        Ubah Password
                    </button>
                </div>
                <div>
